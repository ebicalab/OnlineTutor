    ...
    public class StaticContentExampleHelper : MonoBehaviour {
        ...
        public void Start() {
            ...
            server.SpecialHandlers.NoEndpointFoundHandler = request => {
                ...
                var currentPath = request.RedirectHelper.OriginalPath;
                if (!string.IsNullOrEmpty(request.RedirectHelper.LastRedirectPath)) {
                    currentPath = request.RedirectHelper.LastRedirectPath;
                }
                ...
                Debug.Log($"StaticContentExampleHelper: {currentPath} couldn't be found, redirecting to {redirectPath}");
            };
        }
    }
}